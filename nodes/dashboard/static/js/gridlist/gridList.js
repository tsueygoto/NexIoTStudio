...
GridList.prototype = {
  ...
  toString: function() {
    var widthOfGrid = this.grid.length,
        ...
        i,
        j;
	console.log("kewei widthOfGrid: "+ widthOfGrid);
    ...
    output += '\n';
    console.log("kewei output: "+ output);
    return output;
  },

  generateGrid: function() {
      App.Model.Dashboard.checkSysConfDebug()&&console.log("Kewei : generateGrid");
    /**
    ...
    for (i = 0; i < this.items.length; i++) {
      ...
      // console.log("Kewei Item :"+JSON.stringify(this.items[i]));
    }
  },
  ...
  resizeItem: function(item, size) {
    ...
    getChangedItems: function(initialItems, idAttribute) {
        ...
        */
        ...
        console.info("getChangedItems : ", initialItems, idAttribute);
        ...
        for (i = 0; i < initialItems.length; i++) {
            ...
            if (item.x !== initialItems[i].x ||
                ...
                item.left !== initialItems[i].left ||
                item.top !== initialItems[i].top ||
                item.width !== initialItems[i].width ||
                item.height !== initialItems[i].height ) {
  ...
  _updateItemPosition: function(item, position) {
      App.Model.Dashboard.checkSysConfDebug()&&console.log("_updateItemPosition");
    if (item.x !== null && item.y !== null) {
      this._deleteItemPositionFromGrid(item);
    }
    item.x = position[0];
    item.y = position[1];
    App.Model.Dashboard.checkSysConfDebug()&&console.log(">>> item.x" + item.x);
  ...
  _updateItemSize: function(item, width, height) {
     ...
     */
    App.Model.Dashboard.checkSysConfDebug()&&console.log("_updateItemSize");
    if (item.x !== null && item.y !== null) {
      this._deleteItemPositionFromGrid(item);
    }
    ...
    {
        // TODO: need to design suit for grid method for item.
        // console.info("_markItemPositionToGrid : ", item);
        return;
        ...
        */
        ...
        App.Model.Dashboard.checkSysConfDebug()&&console.log(item.x + item.w);
    
        for (x = item.x; x < item.x + item.w; x++) {
            for (y = item.y; y < item.y + item.h; y++) {
                App.Model.Dashboard.checkSysConfDebug()&&console.log(x);
                this.grid[x][y] = item;
                console.log("Kewei x:"+x+"y:"+y+"this.grid[x][y]:"+this.grid[x][y].toString());
                //this.grid[0][0] = item;
            }
        }
    },
