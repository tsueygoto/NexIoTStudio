...
(function (factory) {
  ...
  DraggableGridList.prototype = {
    ...
    resize: function(rows) {
       App.Model.Dashboard.checkSysConfDebug()&&console.log("resize .........");
      //if (rows) {
    ...
    reflow: function() {
        App.Model.Dashboard.checkSysConfDebug()&&console.log("reflow");
      this._calculateCellSize();
      this.render();
    },

    render: function() {
      App.Model.Dashboard.checkSysConfDebug()&&console.log("[DraggableGridList][render]");
      this._applySizeToItems();
      this._applyPositionToItems();
    },

    add: function( element , w , h , eX , eY ) {
        console.info("extend X Y : ", eX, eY);
        App.Model.Dashboard.checkSysConfDebug()&&console.log("[DraggableGridList][add]");
        ...
        //}
        item = { x : 0 , y : 0 , w : w , h : h, left : 0, top : 0, width : 60, height: 60 };
        ...
        element.attr( {
            "data-x" : (eX === undefined ? nextPos[0] : eX),
            "data-y" : (eY === undefined ? nextPos[1] : eY),
            "data-w" : w,
            "data-h" : h,
            "data-left" : eX,   // the chart init x pos
            "data-top" : eY,    // the chart init y pos
            "data-width" : w,
            "data-height" : h,
        } );
        ...
        return {
            x : (eX === undefined ? nextPos[0] : eX),
            y : (eY === undefined ? nextPos[1] : eY)
        };
    },
    ...
    debug: function() {
      App.Model.Dashboard.checkSysConfDebug()&&console.log( this.gridList.toString() );
    },
    ...
    _updateElementData: function() {
      ...
      for( var i = 0; i < this.items.length; i++ )
      {
        ...
        this.items[i].$element.attr( {
          ...
          "data-w" : this.items[i].w,
          "data-h" : this.items[i].h,
          "data-left" : this.items[i].left,
          "data-top" : this.items[i].top,
          "data-width" : this.items[i].width,
          "data-height" : this.items[i].height
        } );
      }
    },
    ...
    _init: function() {
        App.Model.Dashboard.checkSysConfDebug()&&console.log("_init");
        ...
        console.info("this.items total = ", this.items.length);
        
        
        App.Model.Dashboard.checkSysConfDebug()&&console.dir(this.items);
        ...
        if (this.options.dragAndDrop) {
            ...
            console.info("this.draggableOptions >>>", this.draggableOptions);
    ...
    _bindEvents: function() {
        ...
        this._onAlignChange = this._bindMethod(this._onAlignChange);
        this.$items.on('alignchangestart', this._onStart);
        this.$items.on('alignchangestop', this._onAlignChange);
    },
    ...
    _unbindEvents: function() {
        ...
        this.$items.off('alignchangestart', this._onStart);
        this.$items.off('alignchangestop', this._onAlignChange);
    },
    ...
    _onStart: function(event, ui) {
        ...
        console.info(" item.$element.attr('data-id') : ", item.$element.attr('data-id'));
        
        this.selectedObjs = $("#dashboardPage > ul > li.selected").filter('[data-id!='+item.$element.attr('data-id')+']');
        console.info(this.selectedObjs);
        
        //console.info("all dashboard dom items count : ", this.items.length);
        //this.selected = $("#dashboardPage > ul > li.selected");
        //console.info("all dashboard selected items count : ", this.selected.length);
        //var found = false;
        //var selectedPos = {};
        //for(var i=0; i<this.selected.length; i++)
        //{
        //    if ($(this.selected[i]).is(ui.helper))
        //    {
        //        selectedPos = $(this.selected[i]).position();
        //        console.info("FOUND!!!");
        //        found = true;
        ...
        //    for(var i=0; i<this.selected.length; i++)
        //    {
        //        var pos = $(this.selected[i]).position();
        //        $(this.selected[i]).data('deltaTop', pos.top - selectedPos.top);
        //        $(this.selected[i]).data('deltaLeft', pos.left - selectedPos.left);
        //    }
        //}
    ...
    _moveSelected: function(offsetLeft, offsetTop)
    {
        ...
        //
        //    $this.css('left', pos.left+offsetLeft);
        //    $this.css('top', pos.top+offsetTop);
        //});
    },
    ...
    _onDrag: function(event, ui)
    {
        ...
        //
        //
        console.info("orig :", orig);
        //console.info("currentLoc :", currentLoc);
        
        var item = this._getItemByElement(ui.helper);
        var newPosition = this._snapItemPositionToGrid(item);

        //console.info(item.left, item.top, item.$element.position().left, item.$element.position().top);
        
        
        item.left = item.$element.position().left;
        item.top = item.$element.position().top;
        
        
        var offsetLeft = item.left-orig.left;
        var offsetTop = item.top-orig.top;
        
        //this._moveSelected(offsetLeft, offsetTop);
        var parent = this;
        this.selectedObjs.each(function(){
            $this =$(this);
            
            //console.info("XXXX", $this);
            //console.info("XXXX", $this.attr('data-left'));
            
            //var pos = $this.position();
        
            //var l = $this.pos.left;
            //var t = $this.context.clientTop;
            var item = parent._getItemByElement($this);
            item.left = parseInt($this.attr('data-left'),10)+offsetLeft;
            item.top = parseInt($this.attr('data-top'),10)+offsetTop;
        
            $this.css('left', item.left);
            $this.css('top', item.top);
        });
        
        
        //var deltaLeft = item.left - this.pos.left;
        //var deltaTop = item.top - this.post.top;
        
        console.info("on Drag ...", item.left, item.top, newPosition);
        
        //for(var i=0; (this.selected != null) && (i<this.selected.length); i++)
        //{
        //    //console.info($(this.selected[i]).data('deltaTop'), $(this.selected[i]).data('deltaTop') + item.top);
        //    $(this.selected[i]).css({
        //        top  : $(this.selected[i]).data('deltaTop') + item.top,
        //        left : $(this.selected[i]).data('deltaLeft') + item.left});
        //}
...
//      console.dir(newPosition);
    ...
    _onStop: function(event, ui) {
       ...
       //     //var t = $this.context.clientTop;
       // 
       //     $this.css('left', parseInt($this.attr('data-left'),10)+offsetLeft);
       //     $this.css('top', parseInt($this.attr('data-top'),10)+offsetTop);
       // });
    ...
    _onResize: function( event, ui ) {
      ...
      var itemSize = item.$element.find(".gridItemSize");
      itemSize.html(item.width + "," + item.height);
    ...
    _onResizeStop: function( event, ui ) {
      ...
      var item = this._getItemByElement( ui.helper );
      item.$element.height("100%");
      App.Model.Dashboard.checkSysConfDebug()&&console.log(">>>>> onResizeStop .");

      this._applySizeToItems();
      this._removePositionHighlight();
    },
    
    _onAlignChange: function( event, elem, pos ) {
        //
        var item = this._getItemByElement(elem);
        console.info("item : ", item);
        
        item.top = (pos.t == -1) ? item.top : pos.t;
        item.left = (pos.l == -1) ? item.left : pos.l;
        item.width = (pos.w == -1) ? item.width : pos.w;
        item.height = (pos.h == -1) ? item.height : pos.h
    ...
    _generateItemsFromDOM: function() {
      ...
      this.$items.each(function(i, element) {
        ...
        items.push({
          ...
          h: Number($(element).attr('data-w')),
          id: Number($(element).attr('data-id')),
          left: Number($(element).attr('data-left')),
          top: Number($(element).attr('data-top')),
          width: Number($(element).attr('data-width')),
          height: Number($(element).attr('data-height')),
        });
      });
      return items;
    },
    ...
    _applySizeToItems: function() {
        console.log("[_applySizeToItems] " + this.items.length);
        for (var i = 0; i < this.items.length; i++) {
            App.Model.Dashboard.checkSysConfDebug()&&console.log("[_applySizeToItems] w:" + this._getItemWidth(this.items[i]) + " h:" + this._getItemHeight(this.items[i]));
            
            //App.Debug.log("[_applySizeToItems] w:" + this._getItemWidth(this.items[i]) + " h:" + this._getItemHeight(this.items[i]));
            
            var newh = this._getItemHeight(this.items[i]);
    ...
    _applyPositionToItems: function() {
      ...
      for (var i = 0; i < this.items.length; i++) {
        ...
        App.Model.Dashboard.checkSysConfDebug()&&console.log("x: " + this.items[i].x);
        
        //if( this.options.vertical )
        //{
          App.Model.Dashboard.checkSysConfDebug()&&console.log("####");
          //console.log(this.items[i].x * this._cellHeight);
          //console.log(this.items[i].curX);
          App.Model.Dashboard.checkSysConfDebug()&&console.log("####");
		  
		  var itemLeft = this.items[i].left;
		  var itemTop = this.items[i].top;
		  
		  var snapToGridState = localStorage.getItem("snapToGrid");
		  if (snapToGridState === "true"){
			  itemLeft = 20*(Math.round(itemLeft/20));
			  itemTop = 20*(Math.round(itemTop/20));
		  }
		  else{
			  console.log("kewei left","fail");
		  }
		  
          this.items[i].$element.css({
            left: itemLeft, //.y * this._cellWidth,
            top: itemTop //x * this._cellHeight
          });
    ...
    _snapItemPositionToGrid: function(item) {
      ...
      //console.log("[top, left, buttom, right]=[" + position.top +","+ position.left +","+ position.buttom +","+ position.right + "]");
    ...
    _highlightPositionForItem: function(item) {
      ...
      App.Model.Dashboard.checkSysConfDebug()&&console.log("%%%%");
      App.Model.Dashboard.checkSysConfDebug()&&console.log(item.x);
      App.Model.Dashboard.checkSysConfDebug()&&console.log("%%%%");
      if( this.options.vertical )
      {
        this.$positionHighlight.css( {
          left: item.curY, //.y * this._cellWidth,
          top: item.curX //item.x * this._cellHeight
        } );
      }
      else
      {
        this.$positionHighlight.css( {
          left: item.curX, //.x * this._cellWidth,
          top: item.curY //y * this._cellHeight
        } );
      }
