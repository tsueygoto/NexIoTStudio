#### 2.10.040: Maintenance Release

IoT Studio

- update core/32-udp.* from node-red 0.19 for fix ui operation issue.
 - Fix display error of UDP node editing panel.(OD-169)
 - Fix typos of Merge node.(OD-170)
 - Modify the direction of the Error tooltip.(OD-162)
 - Fix bugs provided by PxC.(OD-163)
	. Remove IoT Studio's Timeout function.
	. Fix typos of Modbus TCP, MOdbus RTU node.
- modify OCD work flow by RESTful method.

Dashboard

 - Fix bugs provided by PxC.(OD-163)
	. The Login page of the dashboard should focus on the username field.
	. Fix the typo of the Dashboard homepage.
 - Fix the abnormality of the pie chart, 3D bar chart, Data table.(OD-168)
 - Fix the bug of the label chart.(OD-163)
 - Modify the chart Information for all charts.(OD-84)
 - Add fields to modify the text color for all charts.(OD-152)
 - Delete unnecessary fields in the charts.(OD-161)
 - Update the NVD3 module to 1.8.6.

#### 2.10.038: Maintenance Release

IoT Studio

 - to improve modbus TCP/RTU in add item to do the value verify.
 - azureiothub: mark amqpWs & fix can't more than one node in flow deploy issue.
 - add nex-node-plc/MITSUBISHI node for MITSUBISHI MELSEC plc.
 - add warning notify for login too many.
 - modbus tcp node : fix when write data is miss or invalid, then node will hang up issue. 


Dashboard

 - fix chart show checkbox in reload didn't show value.
 - plugin_iframe.html: modify for multiple iframes can control rotate to show.
 - show login failed message & show warning message when login fail times over 5.
 

#### 2.00.026: Maintenance Release

IoT Studio

 - modify LicenseRule for OPCUA, only professional can show OpcUa Item & OpcUa Client
 - rename modbusTCP & modbusRTU to remove 'Read' term in node show name
 

Dashboard

 - Add "display grid" and "snap grid" features.
 - Add background settings
	. Image upload
	. Set & clear background
	. Scale & Center
 - Modify the loading page
 - Determine the length of the Image sources list and the data source list according to the user's resolution.
 - Modify the color of all nodes.
 - plugin_image_customer.html
    . Add image upload function
	. Fix the problem that the images list length is too long
 - plugin_liquidfillgauge.html
    . to fix will show alert issue.
    . to fix data update fail issue.
 - plugin_iframe.html
	. Fix an error when adding an iframe chart
	
 - plugin_light.html to add chartTitleColor configure for user setting chart Title color
 - plugin_image_customer.html to hide "Custom Upload" button
 - plugin_gantt.html to add max item count and modify example describe
 - plugin_dataTable.html
    . add chartTitleColor for user configure chart Title color
    . add row padding and font-size configure for user setting row font-size & padding value
    . to add max item count

    
#### 2.00.023: Maintenance Release

IoT Studio

 - Fix modbus TCP/RTU node for jsmodbus & serialport upgrade.
 - Module upgrade:
      jsmodbus : 3.0.0
      serialport: 6.2.0
 - Fix when data is non JSON format will cause azureioteventhub node fail

Dashboard

 - Remove datePicker chart.
 - Label add fontSize item for datasource input


#### 2.00.022: Maintenance Release

 - Add an Account Utility for list/add/del/mod account in IoT Studio/Dashboard

IoT Studio

 - Modify the view show grid and view snap grid in setting configuration
 - Modify security base from MAC to Hard disk serial number
  
 
Dashboard

 - Add reconnect feature when board lost connection
 - Fix new board will lost previous board's plugin info
 - Fix the plugin data table push data issue
 - Remove the unused label field in 3D bard chart
 - fix simple line chart property incorrect problem
 - disable user idle logout timer
 - modify subflow node CSS
 - Remove sentiment node
 - Fix non-hyperx nodes will lose CSS issue
