                ...
                if (data.type == "credentials") {
                    ...
                    $("#node-dialog-login-fields").submit(function(event) {
                        ...
                        $.ajax({
                            ...
                            if (jqXHR.status == 500) {
                                RED.notify( jqXHR.responseJSON.error_description,"warning");
                            }
                            RED.settings.remove("auth-tokens");
...
RED.deploy = (function() {
    ...
    function setdeployInflight(value) {
        deployInflight = value;
    }
    
    return {
        init: init,
        setdeployInflight: setdeployInflight
    }
})();
;/**
 * Copyright JS Foundation and other contributors, http://js.foundation
...
RED.cloudDeploy = (function() {

    var deploymentTypes = { /*kewei modify*/
        "config":{img:"red/images/cloudDeploy-config-o.png"}, 
        "deploy":{img:"red/images/cloudDeploy-deploy-o.png"},
        "link":{img:"red/images/cloudDeploy-link-o.png"}
    }
    ...
    {
        /*kewei modify*/
        if (((type == "config") && (RED.settings.ocdConfStatus == "not ready")) || 
            ((type == "deploy" || type == "link") && (RED.settings.ocdConfStatus != "not ready")))
        {
            deploymentType = type;
            $("#btn-cloudDeploy-icon").attr("src",deploymentTypes[type].img);
            $('#btn-cloudDeploy > span > span').text(function()
            {
                if (type == "config") 
                {
                    $("#btn-cloudDeploy").removeClass('disabled');
                    return RED._("deploy.cloudConfig");
                }
                else if (type == "deploy") 
                {
                    if (cloudDirty)
                    ...
                    {
                        $("#btn-cloudDeploy").addClass('disabled');
                    }
                        
                    return RED._("deploy.cloudDeploy"); 
                }
                else if (type == "link") 
                {
                    $("#btn-cloudDeploy").removeClass('disabled');
                    return RED._("deploy.cloudLink"); 
                }
            });
        }
        else
        {
            RED.menu.setActive("cloudDeploymenu-configuration", false);
            RED.menu.setActive("cloudDeploymenu-deploy", false);
            RED.menu.setActive("cloudDeploymenu-link", false);
            switch(deploymentType)
            {
                case "config":
                    RED.menu.setActive("cloudDeploymenu-configuration", true);
                    break;
                case "deploy":
                    RED.menu.setActive("cloudDeploymenu-deploy", true);
                    break;
                case "link":
                    RED.menu.setActive("cloudDeploymenu-link", true);
                    break;
            }
        }
    }
    ...
    {
        // check 
        if (RED.settings.ocdConfStatus == "ready")
        {
            RED.menu.addItem("btn-deploy-options", null);
            RED.menu.addItem("btn-deploy-options",
            {
                id:"cloudDeploymenu-deploy",
                icon:"red/images/cloudDeploy-deploy.png",
                label:RED._("deploy.cloudDeploy"),
                sublabel:RED._("deploy.cloudDeployDesc"),
                //onselect:function(s) {if(s){changeDeploymentType("deploy")}}
                onselect: "core:do-cloud-deploy"
                //href: "dash/#"
            });
            
            RED.menu.addItem("btn-deploy-options",
            {
                id:"cloudDeploymenu-link",
                toggle:"deploy-type",
                icon:"red/images/cloudDeploy-link.png",
                label:RED._("deploy.cloudLink"),
                sublabel:RED._("deploy.cloudLinkDesc"),
                href: RED.settings.theme("menu.menu-item-help.url", "https://" + RED.settings.ocdConfiscdUrl)
            });
        }
        ...
        //alert(((RED.settings.ocdConfStatus == "not ready") ? "red/images/cloudDeploy-config.png" : "red/images/cloudDeploy-deploy.png"));
        
        //Cloud Deploy Button HTML /*kewei add*/
        //((RED.settings.ocdConfStatus == "not ready") ? RED._("deploy.cloudConfig") : RED._("deploy.cloudDeploy"))
        
        //$('<li><span class="deploy-button-group button-group">'+
        //  '<a id="btn-cloudDeploy" class="deploy-button ' + ((RED.settings.ocdConfStatus == "not ready") ? "" : "disabled") + '" href="#">'+
        //    '<span class="deploy-button-content">'+
        //     '<img id="btn-cloudDeploy-icon" src="' + ((RED.settings.ocdConfStatus == "not ready") ? "red/images/cloudDeploy-config.png" : "red/images/cloudDeploy-deploy.png") + '"> '+
        //     '<span>'+ ((RED.settings.ocdConfStatus == "not ready") ? RED._("deploy.cloudConfig") : RED._("deploy.cloudDeploy")) +'</span>'+
        //    '</span>'+
        //    '<span class="deploy-button-spinner hide">'+
        //     '<img src="red/images/spin.svg"/>'+
        //    '</span>'+
        //  '</a>'+
        //  '<a id="btn-cloudDeploy-options" data-toggle="dropdown" class="deploy-button" href="#"><i class="fa fa-caret-down"></i></a>'+
        //  '</span></li>').prependTo(".header-toolbar");
          
        //Cloud Deploy Menu /*kewei add*/
        var selectedConf = false;
        if (RED.settings.ocdConfStatus == "not ready")
        {
            selectedConf = true;
        }
        ...
        //RED.menu.init(
        //{
        //    id:"btn-cloudDeploy-options",
        //    options: [
        //        {
        //            id:"cloudDeploymenu-configuration",
        //            toggle:"deploy-type",
        //            icon:"red/images/cloudDeploy-config.png",
        //            label:RED._("deploy.cloudConfig"),
        //            sublabel:RED._("deploy.cloudConfigDesc"),
        //            onselect:function(s) 
        //            {
        //                console.log(s);
        //                if(s)
        //                {
        //                    changeDeploymentType("config");
        //                }
        //            }
        //        },
        //        {
        ...
        //                console.log(s);
        //                if(s) {
        //                    changeDeploymentType("link");
        //                }
        //            },
        //            link: "https://" + RED.settings.ocdConfiscdUrl
        //        }
        //    ]
        //});
        ...
        /*Click Deploy Button*/ /*kewei modify*/
        $('#btn-cloudDeploy').click(function(event)
        {
            event.preventDefault();
            // when button is enable state then do action
            // if disable then do nothing
            if (!$(this).hasClass("disabled")) 
            {
        ...
        /*
         *kewei add
         *disable deploy, web link button in the beginning
         */
        $('#cloudDeploymenu-deploy, #cloudDeploymenu-link').addClass("kwTip");
        $('#cloudDeploymenu-deploy > span').attr("data-tooltip",RED._("deploy.cloudDeployToolTip"));
        $('#cloudDeploymenu-link > span').attr("data-tooltip",RED._("deploy.cloudLinkToolTip"));
        //$('#cloudDeploymenu-configuration > span').attr("data-tooltip",RED._("deploy.cloudConfigToolTip"));
        
        
        //RED.menu.setSelected("cloudDeploymenu-configuration", false);
        //RED.menu.setSelected("cloudDeploymenu-link", false);
        //RED.menu.setSelected("cloudDeploymenu-deploy", false);
                
        //if (RED.settings.ocdConfStatus == "not ready")
        //{
        //    deploymentType = "config";
        //    $('#cloudDeploymenu-deploy, #cloudDeploymenu-link').css('opacity','0.2');
        //    $('#cloudDeploymenu-configuration').css('opacity','1');
        //    RED.menu.setSelected("cloudDeploymenu-configuration", true);
        //}
        //else
        //{
        //    deploymentType = "deploy";
        //    $('#cloudDeploymenu-configuration').css('opacity','0.2');
        //    $('#cloudDeploymenu-deploy, #cloudDeploymenu-link').css('opacity','1');
        //    RED.menu.setSelected("cloudDeploymenu-deploy", true);
        //}
        
        RED.actions.add("core:do-cloud-deploy", runOCDFlowsDeploy);
...
//        });
        ...
        /*
		 * When node(s) change
		 * kewei modify 2018/2/23
		 *
		 */
        //RED.events.on('nodes:change',function(state)
        //{
        //    if ((state.dirty) || (cloudDirty))
        //    {
        //        cloudDirty = true;
        ...
        //            }
        //        }
        //        //Cloud Deploy button /*kewei add*/
        //        $("#btn-cloudDeploy").removeClass("disabled");
        //    }
        //    else if ((!cloudDirty) && (deploymentType == "deploy"))
        //    {
        //        window.onbeforeunload = null;
        //        //Cloud Deploy button
        //        RED.menu.setSelected("cloudDeploymenu-configuration", false);
        ...
        // note: notification/runtime-deploy    
        RED.comms.subscribe("notification/runtime-azureconfig",function(topic,msg){
            window.onbeforeunload = null;
            document.location.reload(true);
        });
        ...
        var activeNotifyMessage;
        //RED.comms.subscribe("notification/runtime-clouddeploy",function(topic,msg)
        //{
        //    if (!activeNotifyMessage) {
        //        var currentRev = RED.nodes.version();
        //        if (currentRev === null || deployInflight || currentRev === msg.revision) {
    ...
    function runOCDGwConfigure()
    {
        var promiseAction = new Promise(function(resolve, reject)
        {
            ...
            var aboutHeader = '<div style="text-align:center;"><img width="50px" src="red/images/iot-studio-cloud-dashboard.svg" /></div>';
            var context = 'The GW is **NOT** Azure Config Enabled.\n' +
                        '\n\n' +
                        'Please waiting the donwload link return ![Waiting](./red/images/loading.gif  "waiting")';
        
            RED.sidebar.info.set(aboutHeader+marked(context));
            ...
            $.ajax(
            {
                ...
                //},
                cache: false,
                url: 'oneclickdeployer/ocdgwconfigure',
                success: function(data) 
                {
                    console.info("respnse:", data);
                    RED.sidebar.info.set(aboutHeader+marked(data));
                    RED.sidebar.info.show();
                    RED.notify(RED._("OCDeployer.successfulConfigure"),"success");
                    resolve(RED._("OCDeployer.successfulConfigure"));
                },
                error: function(xhr,textStatus,err)
                {
                    ...
                    if (xhr.status === 401)
                    {
                        context =  RED._("OCDeployer.configureFailed",{message:RED._("user.notAuthorized")});                        //RED.notify(RED._("OCDeployer.configureFailed",{message:RED._("user.notAuthorized")}),"error");
                        errMsg = RED._("user.notAuthorized");
                    } else if (xhr.responseText) {
                        context =  RED._("OCDeployer.configureFailed",{message:xhr.responseText});
                        //RED.notify(RED._("OCDeployer.configureFailed",{message:xhr.responseText}),"error");
                        errMsg = xhr.responseText;
                    } else {
                        context =  RED._("OCDeployer.configureFailed",{message:RED._("OCDeployer.errors.noResponse")});                        //RED.notify(RED._("OCDeployer.configureFailed",{message:RED._("OCDeployer.errors.noResponse")}),"error");
                        errMsg = RED._("OCDeployer.errors.noResponse");
                    }
                    
                    RED.notify(RED._("OCDeployer.configureFailed",{message:errMsg}),"error");
                    RED.sidebar.info.set(aboutHeader+marked(context));
                    RED.sidebar.info.show();
     ...
     *
     */
    function runOCDFlowsDeploy()
    {
        var promiseAction = new Promise(function(resolve, reject)
        {
            // show waiting message
            var aboutHeader = '<div style="text-align:center;"><img width="50px" src="red/images/iot-studio-cloud-dashboard.svg" /></div>';
            var context = 'Do Cloud Flows & Dashboards Deploy.\n' +
                        '\n\n' +
                        'Please waiting the deploying ... ![Deploying](./red/images/loading.gif  "Deploying")';
            RED.sidebar.info.set(aboutHeader+marked(context));
            RED.sidebar.info.show();
            ...
            $.ajax({
                ...
                cache: false,
                url: 'oneclickdeployer/depolyflows2',
                success: function(nodes) 
                {
                    window.onbeforeunload = null;
                    document.location.reload(true);
                    RED.notify(RED._("OCDeployer.successfulDeploy"),"success");
                    resolve(RED._("OCDeployer.successfulConfigure"));
                },
                error: function(xhr,textStatus,err)
                {
                    ...
                    if (xhr.status === 401)
                    {
                        context = RED._("OCDeployer.deployFailed",{message:RED._("user.notAuthorized")});                       //RED.notify(RED._("OCDeployer.deployFailed",{message:RED._("user.notAuthorized")}),"error");
                        errMsg = RED._("user.notAuthorized");
                    } else if (xhr.responseText) 
                    {
                        context = RED._("OCDeployer.deployFailed",{message:xhr.responseText});
                        //RED.notify(RED._("OCDeployer.deployFailed",{message:xhr.responseText}),"error");
                        errMsg = xhr.responseText;
                    } else {
                        context = RED._("OCDeployer.deployFailed",{message:RED._("OCDeployer.errors.noResponse")});                        //RED.notify(RED._("OCDeployer.deployFailed",{message:RED._("OCDeployer.errors.noResponse")}),"error");
                        errMsg = RED._("OCDeployer.errors.noResponse");
                    }
                    RED.notify(RED._("OCDeployer.deployFailed",{message:errMsg}),"error");
                    RED.sidebar.info.set(aboutHeader+marked(context));
                    RED.sidebar.info.show();
        ...
        (
            ...
            {
                //alert("fullfillment: " + value);
                console.info("fullfillment: ", value);
                cloudDirty = false;
                //$("#btn-cloudDeploy").removeClass("disabled");
            },
            // rejection
    ...
    /**
        depends on the global variable deploymentType.
    ...
    function ocdButtonClick()
    {
        switch (deploymentType)
        {
            ...
            case "link":
            {
                var opt = RED.menu.getOpt("cloudDeploymenu-link");
        ...
        //if (deploymentType == "link")
        //{
        //    var opt = RED.menu.getOpt("cloudDeploymenu-link");
        //    window.open(opt.link);
        //    //RED.notify(opt.link,null,true);
        //}
            ...
            nodeEnter.each(function(d,i) {
                    ...
                    if (d._def.button) {
                        ...
                        nodeButtonGroup.append("rect")
							...
							.attr("stroke","#bbb")/*kewei add*/
							.attr("stroke-width",1)/*kewei add*/
                        ...
                        nodeButtonGroup.append("rect")
                            ...
                            .on("mouseup",function(d) {if (!lasso && isButtonEnabled(d)) { d3.select(this).attr("fill-opacity",0.5);d3.event.preventDefault();d3.event.stopPropagation();}})
                            .on("mouseover",function(d) {if (!lasso && isButtonEnabled(d)) { d3.select(this).attr("fill-opacity",0.5);}})
                            .on("mouseout",function(d) {if (!lasso && isButtonEnabled(d)) {
                                var op = 0.05;
                                if (d._def.button.toggle) {
                                    op = d[d._def.button.toggle]?0.05:0.2;
                                }
					...
					var strokeColor = "";
					var	categoryCount = 0;
					var coreCategories = ['subflows', 'input', 'output', 'function', 'DataProcess', 'opcua', 'cloud', 'storage', 'Modbus', 'Config'];
					for (i = 0; i < coreCategories.length; i++){
						if (d._def.category === coreCategories[i]){
							category = d._def.category;
							break;
						}
						categoryCount += 1;
					}
					if (categoryCount >= coreCategories.length){
						fillColor = d._def.color;
						strokeColor = d._def.color;
					}
                    ...
                    var mainRect = node.append("rect")
                        ...
                        .attr("class", "node node_" + category)
                        .on("mouseup",nodeMouseUp)
                        .on("mousedown",nodeMouseDown)
                        .on("touchstart",function(d) {
                            var obj = d3.select(this);
...
RED.palette = (function() {
	...
	/*kewei modify >> Categories Array*/
    var coreCategories = ['subflows', 'input', 'output', 'function', 'DataProcess', 'opcua', 'cloud', 'storage', 'Modbus', 'Config', 'MITSUBISHI'];
    ...
    function addNodeType(nt,def) {
        ...
        if (exclusion.indexOf(def.category)===-1) {
			...
			var categoryCount = 0;
			for (i = 0; i < coreCategories.length; i++){
				if (rootCategory === coreCategories[i]){
					d.className="palette_node palette_node_" + rootCategory;
					break;
				}
				categoryCount += 1;
			}
			if (categoryCount >= coreCategories.length){
				...
				d.style.border = "3px solid " + def.color;
			}
			/*d.className="palette_node";*/

			/*kewei modify("palette_icon_container_"+ rootCategory)*/
            if (def.icon) {
...
RED.remoteDeploy = (function() {
    ...
    function createDialog() {
        dialog = $("<div>",{id:"red-ui-search",class:"red-ui-search"}).appendTo("#main-container");
        var searchDiv = $("<div>",{class:"red-ui-search-container"}).appendTo(dialog);
        searchInput = $('<input type="text" data-i18n="[placeholder]menu.label.remoteDeployInput">').appendTo(searchDiv).searchBox({
            delay: 200,
            change: function() {
                // search($(this).val());
            }
        ...
        searchInput.on('keydown',function(evt) {
            ...
            // if (results.length > 0) {
                ...
                if (evt.keyCode === 13) {
                    ...
                    // if (results.length > 0) {
                    //     // reveal(results[Math.max(0,selected)].node);
                    //     doDeploy(results[Math.max(0,selected)].node);
                    // }
                    doDeploy($(this).val());
                }
            // }
...
//
        ...
        var buttonDiv = $("<div>",{class:"editor-tray-toolbar"}).appendTo(dialog);
        var b = $('<button>').button().appendTo(buttonDiv);
        b.html('Close');
        b.addClass('primary');
        b.click((function(action) {
            return function(evt) {
                if (!$(this).hasClass('disable')) { action(evt); }
            };
        })(function() {
            hide();
        }
    ...
    function doDeploy(vURL) {
        ...
        $.ajax({
            ...
            cache: false,
            url: 'oneclickdeployer/remoteDepoly',
            data: {arrURL:arrURL},
            dataType: "json",
            success: function(nodes) {
                // window.onbeforeunload = null;
                // document.location.reload(true);
                RED.notify(RED._("RemoteDeploy.successfulDeploy"),"success");
                // resolve(RED._("OCDeployer.successfulConfigure"));
            },
            error: function(xhr,textStatus,err) {
                //RED.nodes.dirty(true);
                ...
                //    errMsg = RED._("OCDeployer.errors.noResponse");
                //}
                //RED.notify(RED._("RemoteDeploy.deployFailed",{message:errMsg}),"error");
                //RED.sidebar.info.set(aboutHeader+marked(context));
                //RED.sidebar.info.show();
                
                // reject(errMsg);

                RED.notify(RED._("RemoteDeploy.deployFailed", {
    ...
    function init() {
        RED.actions.add("core:remoteDeploy",show);
