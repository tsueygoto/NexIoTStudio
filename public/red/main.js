...
(function()
{
            ...
            success: function(nodes) {
                ...
                RED.comms.subscribe("status/#",function(topic,msg) {
                    ...
                    if (node) {
                        ...
                        if (msg.hasOwnProperty("key") && msg.hasOwnProperty("keyValue")) {
                            if (node.hasOwnProperty(msg.key))
                            {
                                node[msg.key] = msg.keyValue;
                            }
                        }
                        node.status = msg;
                        node.dirty = true;
    ...
    function loadEditor() {
        ...
        //menuOptions.push({id:"menu-item-node-red-version", label:"v2.00.010", onselect: "core:show-about" });
        var ver;
        console.info(RED.settings);
        ver = (RED.settings.hasOwnProperty('oemversion')) ? RED.settings.oemversion : RED.settings.version;
        menuOptions.push({id:"menu-item-node-red-version", label:"v" + ver, onselect: "core:show-about" });

        menuOptions.push(null);
        menuOptions.push({id:"menu-item-remoteDeploy",label:RED._("menu.label.remoteDeploy"),onselect:"core:remoteDeploy"});
        
        // set default value for view show grid
        if (RED.settings.hasOwnProperty("menu-menu-item-view-show-grid")) {
            RED.settings.set("menu-menu-item-view-show-grid", RED.settings["menu-menu-item-view-show-grid"]);
        }

        // set default value for view snap grid
        if (RED.settings.hasOwnProperty("menu-menu-item-view-snap-grid")) {
            RED.settings.set("menu-menu-item-view-snap-grid", RED.settings["menu-menu-item-view-snap-grid"]);
        }
        ...
        RED.editor.init();
        RED.diff.init();
        RED.remoteDeploy.init();

        RED.menu.init({id:"btn-sidemenu",options: menuOptions});

        RED.deploy.init(RED.settings.theme("deployButton",null));
        
        if (RED.settings.hasOwnProperty("ocdConfStatus") && 
            (RED.settings.ocdConfStatus != "disable"))
        {
            RED.cloudDeploy.init(); 
        }
