...
/**
    check environment is hyperX

 */
function hyperxCheck(settings) {
    if (settings.hasOwnProperty('hyperxScripts')) {
        try {
            var ret = require('child_process').execSync("sh " + (path.join(settings.hyperxScripts, 'hyperX')));
            if ( ret.toString().trim() == "true" ) {
                return true;
            }
        } catch (e) {
            return false;
 ...
 */
function hyperxGetKey(settings) {
    if (settings.hasOwnProperty('hyperxScripts')) {
        try {
            var ret = require('child_process').execSync("sh " + (path.join(settings.hyperxScripts, 'getKey.sh')));
            if (ret) {
...
module.exports = {
    ...
    verifyBuildFolder: verifyBuildFolder,
    hyperxCheck: hyperxCheck,
    hyperxGetKey: hyperxGetKey
};
