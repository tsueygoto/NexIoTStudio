 ...
 */
function requine(file) {
    var r = null;
    
    // check file is exist or not
    if (!fs.existsSync(file)) {
        return {err:file + " file is not exist!"};
    }
    try {
        r = require(file);
        return r;
        ...
        if ((fname != null) && (fs.existsSync(fname))) {
            // remove temp file
            fs.unlink(fname);
        }
        return r;
...
function loadNodeSet(node) {
    ...
    try {
        ...
        if (typeof r === "function") {
            ...
            //console.log("encrypted node.file : ", node.file);
