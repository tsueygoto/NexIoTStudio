 ...
 */
function genOutFormat(cht, content) {
    var line = cht.repeat(content.length + 2);
    return "\n" + line + "\n" + " " + content + "\n" + line + "\n";
}
...
function start(type,diff,muteLog) {
    ...
    if (!muteLog) {
        if (type !== "full") {
            ...
            log.info(genOutFormat('=', log._("nodes.flows.started-flows")));
        }
    }
    return when.resolve();
}
 ...
 * muteLog - don't emit the standard log messages (used for individual flow api)
 */
function setISCDFlows(_config,type,muteLog)
{
    console.info("[setISCDFlows] type:", type, " muteLog : ", muteLog);
...
module.exports = {
    ...
    /**
     * Sets the current active IoT Stdio & Dashbaord config.
     * @param config the configuration to enable
     * @param type the type of deployment to do: full (default), nodes, flows, load
     * @return a promise for the saving/starting of the new flow
     */
    setISCDFlows: setISCDFlows
