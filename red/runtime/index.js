...
function start() {
    return i18n.init()
        ...
        .then(function() {
            ...
            // dump ocd runtime configure
            if (settings.hasOwnProperty('ocdCurConf')) {
                setTimeout(function() {
                    log.info("OCD RunTime Conf\n" + JSON.stringify(settings.ocdCurConf, null, 4));
                }, 3000);
            }
            
            if (require('./util').hyperxCheck(settings)) {
                log.info("HyperX System : true");
            }
            
            if (settings.hasOwnProperty('sn')) {
                log.info("SN : " + settings.sn);
            }
            
            if (settings.hasOwnProperty('licenseType')) {
                log.info("License Type : " + settings.licenseType);
            }
