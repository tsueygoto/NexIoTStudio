...
var passwordTokenExchange = function(client, username, password, scope, done) {
    ...
    if (attemptCount > 5) {
        ...
        done(new Error("Too many login failures. Please try again in 10 minutes."),false);
        return;
    }
    ...
    Users.authenticate(username,password).then(function(user) {
        if (user) {
            ...
            //done(new Error("auth.login.fail.credentials"),false);
            //var err = new Error('Invalid resource owner credentials ' + attemptCount);
