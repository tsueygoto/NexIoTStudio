...
var fileAbsolutePath = path.resolve(argv[3]);
console.log("The encrypt file absolute file path = %s\n", fileAbsolutePath);
if (!fs.existsSync(fileAbsolutePath)) {
    console.log("[ERROR] The encrypt file absolute file path [%s] is not exist.", fileAbsolutePath);
    return;
}
...
try {
    ...
    var content = fs.readFileSync(fileAbsolutePath);
    var before = content.toString('utf8');
    
    // encrypt with Private key
    var encryedContent = privateKey.encryptPrivate(content);
    //console.log(encryedContent.toString('hex'));
    
    fs.writeFileSync('output', encryedContent, 'binary');
    
    //var decryedContent = publicKey.decryptPublic(encryedContent);
    //console.info(decryedContent);
    //var after = decryedContent.toString('utf8');
    //console.info(after);
    //
    //if (content != decryedContent)
    //{
    //    console.info("EQUAL");
    //}
    //else
